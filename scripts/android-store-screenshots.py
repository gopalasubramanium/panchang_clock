...
with (out/'emulator.log').open('w') as log:
    emulator=subprocess.Popen([str(sdk/'emulator/emulator'),'-avd','EksaarStore','-no-window','-no-audio','-no-boot-anim','-no-snapshot','-gpu','swiftshader_indirect','-accel','on'],stdout=log,stderr=subprocess.STDOUT)
    try:
