...
const hash=createHash('sha256');for(const path of paths)hash.update(path).update(await readFile(path));
...
self.addEventListener('fetch',event=>{
 ...
 event.respondWith(caches.open(CACHE).then(async cache=>{
  if(event.request.mode==='navigate'){
   ...
   return (await cache.match(url.href,{ignoreVary:true}))||(await cache.match(new URL('index.html',self.registration.scope).href,{ignoreVary:true}))||fetch(event.request);
  }
