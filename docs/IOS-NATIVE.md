...
Apple rejected iOS 2.0.0 (4) under guideline 4.2 on 23 September 2026. Build 5 replaced the web-shell experience with the native workflows above. Build 6 also keeps city search visible when opening the iPad location picker. Build 7 gives weekday headings unique identities distinct from blank calendar cells, resolving a SwiftUI grid collision found by device tests. Build 7 passed iPhone and iPad Air workflows, but iPad Pro retained a recursive collection layout crash. Build 8 uses eager calendar rows with stable cell heights and makes daily element labels fill their tap areas. It must pass simulator tests and be delivered, attached and resubmitted before its status can be described as submitted. macOS and Android packaging are separate and unchanged by this iOS redesign.
